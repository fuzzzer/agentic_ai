...
class FileManager:
            ...
            if abs_path.startswith(allowed_dir):
                ...
                for pattern in self.forbidden_patterns:
                    if re.search(pattern, path):
                        return {"valid": False, "reason": f"Path matches forbidden pattern: {pattern}"}
                return {"valid": True}
    ...
    def validate_file_content(self, content: str) -> Dict[str, Any]:
        ...
        for pattern in self.forbidden_patterns:
            if re.search(pattern, content):
                return {"valid": False, "reason": f"Content matches forbidden pattern: {pattern}"}
        return {"valid": True}
    ...
    def write_file(self, file_path: str, content: str, 
        ...
        if not content_validation["valid"]:
            return {
                "success": False,
                "error": content_validation["reason"]
            }
    ...
    def update_file(self, file_path: str, pattern: str, 
...
def file_operation(args: Dict[str, Any]) -> str:
    ...
    if operation == "read":
        ...
        if not dir_path:
            return json.dumps({"error": "No directory path provided for list operation"})
