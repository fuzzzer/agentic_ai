...
class AgentHttpService:
    ...
    def __init__(self, base_url=API_BASE_URL, api_key=API_KEY, model_name=MODEL_NAME, tools_description=TOOLS_DESCRIPTION):
    ...
    def send_chat_request(self, user_input):
        ...
        try:
            with httpx.stream("POST", f"{self.base_url}/chat/completions", json=payload, headers=headers, timeout=30.0) as response:
                if response.status_code == 200:
                    return self.process_stream_response(response)
                else:
                    # Read the content before accessing it
                    content = response.read()
                    error_msg = f"❌ API Error: {response.status_code} - {content.decode()}"
    ...
    def process_stream_response(self, response) -> str:
        ...
        for line in response.iter_lines():
            try:
                str_line: str = line
                json_str: str = str_line[len("data: "):].strip() if str_line.startswith("data: ") else str_line.strip()
                ...
                if "choices" in parsed_line and len(parsed_line["choices"]) > 0:
                    delta_content: str = parsed_line["choices"][0]["delta"].get("content", "")
