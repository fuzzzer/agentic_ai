...
from agent.components.description import LM_STUDIO_TOOLS, TOOLS_DESCRIPTION
...
class AgentOpenAIService(AgentService):
    def __init__(
        ...
        logger.info(
            "nLOG: Initialized AgentOpenAIService with model: %s", self.model_name
        )
    ...
    def _stream_until_tool_or_end(self, messages):
        ...
        if not full_tool_call_str:
            full_text, full_tool_call_str = self._extract_tool_command(
                full_text
            )  # check for any tool identifiers for the models that manually use tools
    ...
    def chat_with_model(self, user_input: str, user_role: str) -> str:
        ...
        while iterations < max_iterations:
            ...
            try:
                ...
                self.conversation_history.append(
                    {"role": "tool", "content": f"Tool result: {sanitized_result}"}
                )
                ...
                self.conversation_history.append(
                    {"role": "tool", "content": f"Tool execution error: {error_msg}"}
                )
